---
title: "Forecasting Demand for Bike share in Boston"
...
Our analysis of bike demand will include all stations in the Boston Bluebikes network except for bike stations located in Salem and the bike station located at the Blue Cross Blue shield of in Hingham. These stations are excluded because they are located very far away from the other stations in Bluebikes network.  

The model presented here is designed to predict demand over summer months, and was trained on bike trip data covering a five week period from June 25th 2023 to July 31st 2023. It is important to note that this model should only be used to predict demand in the summer months due to the training data used. The model is not generalizable to the winter months.
...
Here we import the bike share data from BlueBikes
...
Count the number of bike share rides occurring per hour by station.
...
Join census data to bike share data. Filter out bike share stations which are not part of analysis.
...
The first chart shows below the number of rides taking place per day across the BlueBike network broken down by time of day. The number of rides which occur per day is generally around 12,500 to 15,000 rides. Notably, the total number of rides declines sharply on days when heavy rain storms occurred. Examples of this include July 2nd, July 10th, July 16th, and July 29th. The total number of rides occurring on weekend days is similar to the total number of rides occurring on weekdays.
...
Our next step is to run a k-fold cross validation. A cross validation allows for determining if the model produces similar results when trained using a different data sample. A model cross validation involves training the model using randomly selected training and testing data. Each set of training and testing data is called a fold. The analysis includes a twenty fold cross validation. For each fold, the model is trained using the training dataset and predictions for the number of bike share trips are predicted for the data points in the testing dataset. 
...
reg.cv <- train(sum_rides ~  hour(interval60) + dotw + time_of_day + Name + District + Precip + clean_lag + holiday,  
                data=all_data %>% st_drop_geometry() %>% dplyr::select(sum_rides, interval60, dotw, time_of_day, Name, District, Precip, clean_lag, holiday),
                method = "lm", trControl = fitControl, na.action = na.pass)

```

```{r cross_validation_read, echo=FALSE}
cv_results <- read.csv('./Data/cv_results.csv')
```

The chart below show the results of the cross validation for all 20 folds. The Mean absolute error  for all folds is slightly above 1 for all folds indicating that the model provides consitent results when trained on different data.  

``` {r cross_validate_results, fig.width=10, fig.height=5}

ggplot()+
  geom_histogram(data=cv_results,aes(x=MAE),fill='orange',color='grey20',bins=50)+
  xlim(0,2)+
