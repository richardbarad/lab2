...
```{r data visualization categorical, fig.height=7,fig.width=10}
...
set.seed(3456)
housing_index <- createDataPartition(housing_data$y_numeric, p = .65,
                                  list = FALSE,
                                  times = 1)

housing_train <- housing_data[housing_index,] %>%  transform(y_numerc = as.numeric(y_numeric)) 
housing_test  <- housing_data[-housing_index,] %>% transform(y_numerc = as.numeric(y_numeric))
```
